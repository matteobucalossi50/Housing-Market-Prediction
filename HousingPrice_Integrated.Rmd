...
kc_house_data <- subset(kc_house_data, kc_house_data$bedrooms != 0 | kc_house_data$bathrooms != 0) 
...
price_minus_outliers <- outlierKD(kc_house_data, price)
str(price_minus_outliers)
...
sqft.lot_nooutliers <- cut(price_minus_outliers$sqft_lot, breaks = c(519,330688,660856,991023,1321191, 1651359), labels = c("520-330K", "330K-660K", "660K-991K", "991K-1.3M", "1.3M-1.65M"))


ggplot(price_minus_outliers, aes(x=sqft.lot_nooutliers, y=price, fill=sqft.lot_nooutliers)) + geom_boxplot() + scale_fill_brewer(palette="Spectral") + ggtitle("Housing Price vs. Land Area (Outliers Removed)") + ylab("Housing Price ($)") + xlab("Land Area (sqft.)") +  theme(plot.title= element_text(hjust=0.5, size = 14)) + scale_y_continuous(labels = comma) 
...
ggplot(kc_house_data, aes(x=condition, y=price_minus_outliers$price, fill=condition)) + geom_boxplot() + scale_fill_brewer(palette="Spectral") + ggtitle("Housing Price vs. Apartment Condition (Outliers Removed)") + ylab("Housing Price ($)") + xlab("Apartment Condition") +  theme(plot.title= element_text(hjust=0.5, size = 14)) + scale_y_continuous(labels = comma) 
...
ggplot(price_minus_outliers, aes(x=grade, y=price, fill=grade)) + geom_boxplot() + scale_fill_brewer(palette="Spectral") + ggtitle("Housing Price vs. Apartment Grade (Outliers Removed)") + ylab("Housing Price ($)") + xlab("Apartment Grade") +  theme(plot.title= element_text(hjust=0.5, size = 14)) + scale_y_continuous(labels = comma) 
...
number.bedrooms <- cut(price_minus_outliers$bedrooms, breaks = c(-1,3,6,9,33), labels = c("0-2", "3-5", "6-8", "9-33"))

ggplot(price_minus_outliers, aes(x=number.bedrooms, y=price, fill=number.bedrooms)) + geom_boxplot() + scale_fill_brewer(palette="Spectral") + ggtitle("Housing Price vs. # Bedrooms (Outliers Removed)") + ylab("Housing Price ($)") + xlab("# Bedrooms") +  theme(plot.title= element_text(hjust=0.5, size = 14)) + scale_y_continuous(labels = comma) 
...
year.built <- cut(price_minus_outliers$yr_built, breaks = c(1899,1923,1946,1969,1992,2015), labels = c("1900-1923", "1924-1946", "1947-1969", "1970-1992", "1993-2015"))

ggplot(price_minus_outliers, aes(x=year.built, y=price, fill=year.built)) + geom_boxplot() + scale_fill_brewer(palette="Spectral") + ggtitle("Housing Price vs. Year Built (Outliers Removed)") + ylab("Housing Price ($)") + xlab("Year Built") +  theme(plot.title= element_text(hjust=0.5, size = 14)) + scale_y_continuous(labels = comma) 
