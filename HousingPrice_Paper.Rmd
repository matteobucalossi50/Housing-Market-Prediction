...
TKcond
```
Note that the p-value of BP test are `lot_p$p.value`, which is greater than 0.05. Thus, the variances are the same for different groups. Then we use ANOVA test analysis the differences of mean prices. Since the p-value is `r land.anova` which is lower than 0.05, we need to reject the null hypothesis. The mean prices of the different sqft.lot levels are different. From the Turkey test we can see, the p-value of the 660k-991k and 520k-330k, 660k-991k and 330K-660K, 660k-991k and 991K-1.3M are the lowest three. And the p-value for other pairs are very high, which means the means price of 660k-991k(the median level) are higher than others'. Thus the land area of 660k-991k are the most popular level for houses in WA. 
...
ggplot(kc_house_data, aes(x=year.built, y=price, fill=year.built)) + geom_boxplot() + scale_fill_brewer(palette="Spectral") + ggtitle("Housing Price vs. Year Built (log)") + ylab("Housing Price ($)") + xlab("Year Built") +  theme(plot.title= element_text(hjust=0.5, size = 14)) + scale_y_continuous(labels = comma) + scale_y_log10(labels = function(x) format(x,nsmall = 2,scientific = FALSE, big.mark = ','))
...
ggplot(norenov, aes(x=year.built.noren, y=price, fill=year.built.noren)) + geom_boxplot() + scale_fill_brewer(palette="Spectral") + ggtitle("Not Renovated Housing Price vs. Years Built(log)") + ylab("Housing Price ($)") + xlab("Year Built") +  theme(plot.title= element_text(hjust=0.5, size = 14)) + scale_y_log10(labels = function(x) format(x,nsmall = 2,scientific = FALSE, big.mark = ',')) 
...
```
Since p-value is `r chisq.gradey`, which is lower 0.05, we need to reject the null hypothesis. Thus, the year built and grade are not independent. They are corrlated.  
...
```
Since p-value is `r chisq.condy`, which is lower 0.05, we need to reject the null hypothesis. Thus, the year built and grade are not independent. They are corrlated.  
...
```
Since p-value is `r chisq.renov`, which is lower 0.05, we need to reject the null hypothesis. Thus, the year built and grade are not independent. They are corrlated.  
