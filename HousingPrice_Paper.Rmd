...
Now we want to see whether condition and grade are related; for this, we must perform a chi-square test.
```{r chisq grade}
...
```
Since the p-value is `r chisq.cond$p.value`, which is lower than 0.05, we need to reject the null hypothesis. Thus, condition and grade are not independent. They are correlated. Generally, grade is better when condition is better.
...
Here, we perform the BP test to test the homoscedasticity for different yr_built groups.

H0: Price variance is the same across the different yr_built groups.  
H1: Price variance is not equal across different yr_built groups.  

```{r anova year}
yr_p <- bptest(kc_house_data$price~year.built)
yr_p
```
Since the p-value is `r yr_p$p.value`, which is lower than 0.05, we need to reject the null hypothesis. The variances for different yr_built groups are not equal. Thus, the ANOVA test is not applicable.  
...
chisq.gradey
```
Since the p-value is `r chisq.gradey$p.value`, which is lower 0.05, we need to reject the null hypothesis. Thus, the year built and grade are not independent. They are correlated.  
...
chisq.condy
```
Since the p-value is `r chisq.condy$p.value`, which is lower 0.05, we need to reject the null hypothesis. Thus, the year built and condition are not independent. They are correlated.  
...
chisq.renov
```
Since the p-value is `r chisq.renov$p.value`, which is lower 0.05, we need to reject the null hypothesis. Thus, condition and yr_renovated are not independent. They are correlated.  
