...
nrow(kc_house_data[kc_house_data$sqft_living >= 10891, ])
...
nrow(kc_house_data[kc_house_data$sqft_lot >= 991000, ])
...
Here, we have a logarithmic box plot of housing price vs. "bedrooms". There appears to be a clear trend: as the number of bedrooms increases, housing price increases as well. The 9-11 interval bucks the trend slightly, but again, this can be explained by the fact that only 10 houses are part of this interval, compared with 21,586 total for the others.

```{r, echo=FALSE}

nrow(kc_house_data[kc_house_data$bedrooms >= 9, ])

nrow(kc_house_data[kc_house_data$bedrooms <= 8, ])
...
Here, we've graphed housing price by yr_renovated itself. These graphs also showcase only 914 properties – the ones that were renovated. Generally speaking, as "yr_renovated" approaches the present day, price increases. Expception is between the 1924-1946 and 1947-1969 intervals; note however, that only properties occupy the first interval.
...
nrow(kc_house_data[kc_house_data$yr_renovated >0 & kc_house_data$yr_renovated <= "1946", ])

renov.old <- subset(renov, renov$yr_built < 1950)
price.ln.renold = log(renov.old$price)
year.renov.renold <- cut(renov.old$yr_renovated, breaks = c(1899,1923,1946,1969,1992,2015), labels = c("1900-1923", "1924-1946", "1947-1969", "1970-1992", "1993-2015"))

ggplot(renov.old, aes(x=year.renov.renold, y=price.ln.renold, fill=year.renov.renold)) + geom_boxplot() + scale_fill_brewer(palette="Spectral") + ggtitle("Old Housing Price vs. Years Renovated(log)") + ylab("Housing Price ($)") + xlab("Year Renovated") +  theme(plot.title= element_text(hjust=0.5, size = 14)) + scale_y_continuous(labels = comma) 

renov.new <- subset(renov, renov$yr_built > 1950)
price.ln.rennew = log(renov.new$price)
year.renov.rennew <- cut(renov.new$yr_renovated, breaks = c(1899,1923,1946,1969,1992,2015), labels = c("1900-1923", "1924-1946", "1947-1969", "1970-1992", "1993-2015"))

ggplot(renov.new, aes(x=year.renov.rennew, y=price.ln.rennew, fill=year.renov.rennew)) + geom_boxplot() + scale_fill_brewer(palette="Spectral") + ggtitle("New Housing Price vs. Years Renovated(log)") + ylab("Housing Price ($)") + xlab("Year Renovated") +  theme(plot.title= element_text(hjust=0.5, size = 14)) + scale_y_continuous(labels = comma)
