...
hist(kc_house_data$price, xaxt="n", ylim = c(0,14000), col = "green3", main = "Housing Price Histogram", xlab = "Housing Price ($)", cex.axis = .75)
...
date_p$p.value
```
Since the p-value is `r date_p$p.value` which is lower than 0.05, we need to reject the null hypothesis. The variances for different sqft of living levels are different. Thus, the ANOVA test is not applicable.  
...
TKcond
```
Note that the p-value of BP test are `r lot_p$p.value`, which is greater than 0.05. Thus, the variances are the same for different groups. Then we use ANOVA test analysis the differences of mean prices. Since the p-value is `r land.anova$p.value` which is lower than 0.05, we need to reject the null hypothesis. The mean prices of the different sqft.lot levels are different. From the Turkey test we can see, the p-value of the 660k-991k and 520k-330k, 660k-991k and 330K-660K, 660k-991k and 991K-1.3M are the lowest three. And the p-value for other pairs are very high, which means the means price of 660k-991k(the median level) are higher than others'. Thus the land area of 660k-991k are the most popular level for houses in WA. 
...
Then we do BP Test to see the homoscedasticity of the different groups.
...
bed_p
```
Since the p-value is `r bed_p$p.value` which is lower than 0.05, we need to reject the null hypothesis. The variances for different number of bedrooms groups are different. Thus, the ANOVA test is not applicable.  
...
Since the anova test is not applicable, we want to use chi-square test to see the corrleation between the number of bedrooms and the house price. The prices and number of bathrooms are divided into different groups by the cut function to make it categorical so that we can do chi-square test for them.
...
## SMART Question: **Are houses of different quality priced differently?** *check all these questions make sense given what nick said*
...
cp
```
Since the p-value is `r cp$p.value`, which is greater than 0.05. We failed to reject the null hypothesis. The variances of price between different condition group are the same. The ANOVA test is applicable.
...
TKcond
```
Since the p-value is `r cond.anova$p.value`, which is lower than 0.05. We need to reject the null hypothesis. The mean prices of the different condition are different. From the Tukey test we know, the condition 1 and 2 has a very large p-value. Other p-values are very low. Basically, the price increases when the condition is better.  
...
ggplotly(ggplot(kc_house_data, aes(x=grade, y=price, fill=grade)) + geom_boxplot() + scale_fill_brewer(palette="Spectral") + ggtitle("Housing Price vs. Apartment Grade (log)") + ylab("Housing Price ($)") + xlab("Apartment Grade") +  theme(plot.title= element_text(hjust=0.5, size = 14)) + scale_y_continuous(labels = comma) + scale_y_log10(labels = function(x) format(x,nsmall = 2,scientific = FALSE, big.mark = ',')))
...
gp
```
Since the p-value is `r gp$p.value`, which is lower than 0.05, we need to reject the null hypothesis. The variances of prices for different grades are different. Thus, the ANOVA test is not applicable.  
 


*explains chi-sq results and how they related*
Then we want to see the condition and the grade are related. So we do the chi-square test to see if they related or not.
```{r chisq grade}
...
```
Since the p-value is `r chisq.cond$p.value`, which is lower than 0.05. We need to reject the null hypothesis. Thus, the condition and grade are not independent. They are corrlated. Gerenally,  the grade is better when the condition is better.
...
ggplotly(ggplot(kc_house_data, aes(x=year.built, y=price, fill=year.built)) + geom_boxplot() + scale_fill_brewer(palette="Spectral") + ggtitle("Housing Price vs. Year Built (log)") + ylab("Housing Price ($)") + xlab("Year Built") +  theme(plot.title= element_text(hjust=0.5, size = 14)) + scale_y_continuous(labels = comma) + scale_y_log10(labels = function(x) format(x,nsmall = 2,scientific = FALSE, big.mark = ',')))
...
*no anova..*
We do BP test to the homoscedasticity for different year of built groups.
...
yr_p
```
Since the p-value is `r yr_p$p.value`, which is lower than 0.05, we need to reject the null hypothesis. The variances for different year built groups are different. Thus, the ANOVA test is not applicable.  
...
t
```
Since the p-value is `r yr_R.anova$p.value`, which is less than 0.05. We need to reject the null hypothesis. Thus, the prices of the renovated are different accross the year of built. From the Turkey, we can see that the mean price of renovated hosue from 1947 to 1969 and 1924 to 1946 are different from each other. Same as the pair that built from 1970 to 1992 and 1924 to 1946. 
*do anova for this one too*


**compared renovated with built and see price changes too..**

## SMART Question: **Is quality affected by house age?**

### Chi-square test for year they were built and grade
*use chisq to assess old and quality independence*

#### Chi-square test for grade and year of built.  

H0: The grade and year of built are independent.   
H1: The grade and year of built are in not independent.   
...
chisq.gradey
```
Since p-value is `r chisq.gradey$p.value`, which is lower 0.05, we need to reject the null hypothesis. Thus, the year built and grade are not independent. They are corrlated.  

#### Chi-square test for condition and year of built.

H0: The condition and year of built are independent.   
H1: The condition and year of built are not independent.   
```{r chisq year 2}
...
chisq.condy
```
Since p-value is `r chisq.condy$p.value`, which is lower 0.05, we need to reject the null hypothesis. Thus, the year built and grade are not independent. They are corrlated.  
...
#### Chi-square test for condition and year of renovated.  

H0: The condition and year of renovated are independent.   
H1: The condition and year of renovated are not independent.   
...
chisq.renov
```
Since p-value is `r chisq.renov$p.value`, which is lower 0.05, we need to reject the null hypothesis. Thus, the year built and grade are not independent. They are corrlated.  
...
In conclusion, this analysis provided many insights into the dynamics of the housing market in the Seattle area. After conducting EDA on most of the variables in the dataset and considered possible interactions among them, we were able to narrow down better questions to assess what could actually cause price fluctuations for houses in the area. In particular, we realized how few houses were renovated (or at least recorded as such), providing us no explanation for the not so great difference in prices between older and newer house. Most likely, we suppose older houses may have some historical values thus a comparable price to the newer buildings.  
As we proceeded with further hypothesis analysis, we were able to confirm the expected answers to most of our questions, as price resulted influenced by sqft, quality and in some way age. Finally, our regression model uncovered further elements to predict housing price on several variables present in the dataset, making it easier for future consultancy and possible to run through future datasets and adjust for better predictive power.  
We believe that additional and more extensive information on renovation may have provided useful insights to assess the actual correlation between age and price of houses. Further details on certain more unclear variables could have also made this analysis and the data behind it stronger and more replicable. Further study would have the possibility to examine the ... as the market in the area proves to be booming, and also particularly impactful for many data scientists moving to WA for employment opportunities.
