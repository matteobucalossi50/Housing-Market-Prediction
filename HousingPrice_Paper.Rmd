...
TKcond
```
Note that the p-value of BP test are `r lot_p$p.value`, which is greater than 0.05. Thus, the variances are the same for different groups. Then we use the ANOVA test to analyze the differences of mean prices. Since the p-value is `r land.anova$p.value`, which is lower than 0.05, we need to reject the null hypothesis. The mean prices of the different sqft.lot levels are different. From the Turkey test, we can see that the p-values for the following pairs are the lowest: the 660k-991k and 520k-330k, 660k-991k and 330K-660K, 660k-991k and 991K-1.3M. And the p-value for other pairs are very high, which means the mean price of 660k-991k(the median level) are higher than others'. Thus, the land area of 660k-991k are the most popular level for houses in WA. 
...
```
Next, we perform the BP Test to determine the homoscedasticity of the different groups.
...
Since the anova test is not applicable, we want to use chi-square test to see the corrleation between the number of bedrooms and house price. The prices and number of bathrooms are divided into different groups by the cut function to make it categorical, so we can perform the chi-square test on them.
...
TKcond
```
Since the p-value is `r cond.anova$p.value`, which is lower than 0.05, we need to reject the null hypothesis. The mean prices of the different conditions are not equal. From the Tukey test we know that conditions 1 and 2 have very large p-values. Other p-values are very low. Basically, the price increases when the condition is better.  
...
H1: The variances of prices are not eqal across different grades.  

```{r anova grade, include=FALSE}
gp <- bptest(price~grade, data = kc_house_data)
gp
```
Since the p-value is `r gp$p.value`, which is lower than 0.05, we need to reject the null hypothesis. The price variance for different grades are not equal. Thus, the ANOVA test is not applicable.  
 

Now we want to see the condition and the grade are related. So we do the chi-square test to see if they related or not.
```{r chisq grade}
...
```
Since the p-value is `r chisq.cond$p.value`, which is lower than 0.05, we need to reject the null hypothesis. Thus, condition and grade are not independent. They are correlated. Gerenally, grade is better when condition is better.
...
We do BP test to test the homoscedasticity for different yr_built groups.

H0: Price variance is the same across the different yr_built groups.  
H1: The variances of prices are not equal across different yr_built groups.  

```{r anova year}
yr_p <- bptest(kc_house_data$price~year.built)
yr_p
```
Since the p-value is `r yr_p$p.value`, which is lower than 0.05, we need to reject the null hypothesis. The variances for different yr_built groups are not eqal. Thus, the ANOVA test is not applicable.  
...
t
```
Since the p-value is `r yr_R.anova$p.value`, which is less than 0.05, we need to reject the null hypothesis. Thus, the prices of the renovated are different across yr_built. From the Tukey test, we can see that the mean price of renovated houses from 1947-1969 and 1924-1946 are different from each other. The same goes for the pair 1970-1992 and 1924-1946. 

## SMART Question: **Is quality affected by house age?**

### Chi-square test for yr_built and grade

H0: Grade and yr_built are independent.   
H1: Grade and yr_built are not independent.   
...
chisq.gradey
```
Since p-value is `r chisq.gradey$p.value`, which is lower 0.05, we need to reject the null hypothesis. Thus, the year built and grade are not independent. They are correlated.  

#### Chi-square test for condition and yr_built.

H0: Condition and yr_built are independent.   
H1: Condition and yr_built are not independent.   
```{r chisq year 2}
...
chisq.condy
```
Since p-value is `r chisq.condy$p.value`, which is lower 0.05, we need to reject the null hypothesis. Thus, the year built and condition are not independent. They are correlated.  

### Does quality get better when renovated?

#### Chi-square test for condition and year of renovated.  

H0: Condition and yr_renovated are independent.   
H1: Condition and yr_enovated are not independent.   
...
chisq.renov
```
Since p-value is `r chisq.renov$p.value`, which is lower 0.05, we need to reject the null hypothesis. Thus, condition and yr_renovated are not independent. They are correlated.  
