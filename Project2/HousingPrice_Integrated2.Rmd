...
# PCA
##Use principle component analysis to predict the house price. Choosing this method because there are too many variables and I did not know which one to use so I want to capture as most as possible information by the least variables. 
...
##Firstly， I loaded the dataset into Rstudio, renamed it to house and took a look at the structure of the dataset.
...
##subset data 
##I delete some unuseful variables which I think are uncorrelated to the house price like lattitude, longitude, the 15 neighborhoods' sqft_living , sqft_lot, the year of renovated, zipcode, date of record, id. Also there are serveral variables contain mostly 0 values so I also deleted them like view and waterfront. And then took a look at the dataset

```{r subs}
house2 <- subset(house,select = -c(id,date,waterfront,view,zipcode,lat,long,sqft_living15,sqft_lot15,yr_renovated))
str(house2)
```
##There are 11 variables left and one variable is house which will be used as independent variable and all variables are numeric. 
...
corx
```
##Took a look at the correlation of the 10 dependent variables and I found that they are all related, some ones are highly related. It shows me that very hard to pick which ones are important so I used PCA to reduce dimension. 



##PCA part,scale=T normalize the variables becuase they have different scales.As the 10 variable have different scales, it is necessary to scale them before analysis. Performing PCA on un-normalized variables will lead to insanely large loadings for variables with high variance.
##Then use prcomp function to do the PCA. Checked the mean and sd of variables. As I scaled the variables, so the differences in mean and sd are not big. 
```{r pca}
...
```
##We can see that with 7 components, there are 95% cumulative variances explained. 95% percentage is a normal for PCA, so I decided to use 7 components. 

##Take a look at how variables form up each component. 
```{r rot}
prin_comp$rotation
```
##We can see that sqft_above forms up nearly 43% of the first component which is the highest. For the second component, sqft_basement forms up mostly, which is nearly 60%. 

##We can visualize the variance explained by each component. We can see that the first component explains mostly, the later ones explain less. 
```{r eig}
fviz_eig(prin_comp)
```

##We can visualize how each component formed up by different variables. However, the graph is very hard to see, not as easy to see as rotation report. 
```{r bip}
...
##Visulize the cumulative proportions of variance, we can see that after 7 components, the curve becomes smooth. 
```{r pve}
...
##Predicting use pca
##Finally, we can created a pcr model wth the 7 components. Actually, the PCR model is a linear model, so I used lm function to create it. 
```{r predpca}
houseprice <- prin_comp$x
modHouses <- lm(house2$price ~ houseprice[,1:7])
summary(modHouses)
```
##We can see that there is 61.3% variance explained by the independent variable. I do not use accuracy as a scale to check whethere the model is good or not. The reason is that the difference between the house price is very large. So using accuracy to check the model is unreachable. 

##I want to see how good the PCR model is, so I created a full linear model to compare. 
```{r fullpca}
fullmodel=lm(price~.-sqft_basement,data=house2)
summary(fullmodel)
```
##There is 61.8% variance explained by the independent variable in the full linear model. It is little better than the PCR model. 
...
```
##Create a picture to compare the PCR model with the full linear model. 
...
#There are few outliers of the price. I do not delete them because I think these high price are also important. 
#The response variables must be numeric, even the grade and condition variables are actually catagorical. 
#PCA relies on normality assumption
#The components must be orthogonal (Sometimes consider that principal components are orthogonal to the others it's a restriction to find projections with the highest variance)
...
We first load the dataset and remove columns that would be used in the regression.
Then drop the NA values from the dataset.
```{r data clean}
kc_house_data <- read.csv('kc_house_data.csv')
...
### The Ridge 
For our dataset, numbers of bedrooms, bathrooms, floors are categorical variables. So we transfer them into factors.
Then we prepare log scale grid for λ values, from 10^10 to 10^-2, in 100 segments and build the ridge model.
Afterwards, we draw plot of coefficients to see the overall trend. 
```{r splitridge}
loadPkg("ISLR")
...
There are 55 coefficients for each model. The 100 $\lambda$ values are chosen from 0.02 ($10^{-2}$) to $10^{10}$, essentially covering the ordinary least square model ($\lambda$ = 0), and the null/constant 
...
```
The test set mean-squared-error MSE is `r mean((ridge.pred-y_test)^2)` (remember that we are using standardized scores) for $\lambda = 4$. 

```{r mseridge}
mean((mean(y_train)-y_test)^2) # the test set MSE
```
On the other hand, for the null model ($\lambda$ approach infinity), the test MSE can be found to be `r mean((mean(y_train)-y_test)^2)`. So $\lambda = 4$ reduces the variance by about half at the expense of the bias.
...
Now for the other extreme speical case of small $\lambda$, which is the ordinary least square (OLS) model. We can first use the ridge regression result to predict the $\lambda$ =0 case. The MSE found to be `r mean((ridge.pred - y_test)^2)` that way. 
...
rsq
```
The first verticle dotted line is shows the lowest MSE is `r mean((ridge.pred-y_test)^2)`. The second verticle dotted line is within one standard error. Then we calcualte the R square value. The R square is `r rsq` for ridge model.

### The Lasso

The same functino glmnet( ) with alpha set to 1 will build the Lasso regression model. 
Then draw the polt for different $\lambda$ values to see the overall trend.  
```{r lasso}
...
plot(cv.out)
```
Here, we see that the lowest MSE is when $\lambda$ appro = 0.369. It has about 47 non-zero coefficients. 
...
rsq1
```
Then calculate the R square of lasso regression which is `r rsq1`.  

Lasso regression is also a good tool for features selection. So we build a linear model by using lasso to select variables.
```{r lmlasso}
...
The p-value of conditions are all lower than 0.05. Remove it from the model and rebuild.
...
vif(lasLin)
```
The R square is `r summary(lasLin1)$adj.r.squared` which is better than ridge and lasso regression.
