...
```{r test, echo=FALSE}
fit <- rpart(log(price)~bedrooms + bathrooms + sqft_living + sqft_lot + floors + condition + grade + sqft_above + sqft_basement+yr_built+yr_renovated, data=train.set)
...
```
```{r}
treepr <- prune(fit, best = 5, cp=.02) # Return best pruned tree with 5 leaves, evaluating error on training data
summary(treepr)
plot(treepr) 
text(treepr,cex=0.75, digits=3)

tree.pred.seq <- prune(treepr, cp=.02)
plot(tree.pred.seq)
tree.pred.seq$dev
...
tree.pred <- rpart(log(price)~bedrooms + bathrooms + sqft_living + sqft_lot + floors + condition + grade + sqft_above + sqft_basement+yr_built+yr_renovated, data=train.set, cp=.02)
treepr.pred <- prune(tree.pred, best = 5, newdata = test.set, cp = 0.02) #evaluates on test data
treepr.pred
plot(treepr.pred) 
...
```{r acctr, echo=FALSE}
#rsq.rpart(tree.pred)
#p1 <- predict(tree.pred, test.set)
#MAE <- function(actual, predicted) {mean(abs(actual - predicted)) }
#MAE(test.set$price, p1)

tr.pred = predict(fit, s = 0, newx = train.set)
mean((tr.pred - test.set$price)^2)
#predict(fit, s = 0, type="coefficients")[1:12,]
```
